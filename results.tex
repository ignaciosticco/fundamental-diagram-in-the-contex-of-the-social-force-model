...
In this Section we present the results relating the local flow, velocity and density (\textit{i.e.} the fundamental diagram). The measurements were taken in the middle of the corridor using the definitions given in Eq.~(\ref{ec-v}) and Eq.~(\ref{ec-flow}), and as shown in Fig.~\ref{pasillo}. All the results shown here correspond to $R=1$~m (see Eq.\ref{ec-flow}). We further barred $R$ until $R=3$, but no significant changes were observed. \\
...
of the corridor once the system reached the stationary state (see Fig.~\ref{pasillo}). The length of the corridor 
...
As we mentioned in Section \ref{fundamental_diagram}, when the density is low, pedestrians achieve the desired velocity ($v=v_d=1$~m/s). Since the results of the previous section only hold for the area located in the middle of the corridor (see Fig.\ref{pasillo}), we want to shed some light and understand what is happening across the entire corridor.\\
