...
data_22m = np.genfromtxt("flow-density_vd1_pasillo22m.txt",delimiter = ' ') 
density_22m = data_22m[:,0]
flow_22m = data_22m[:,1]

data_koriginal = np.genfromtxt('fis_225p_d1.84.txt', delimiter = ' ')
...
data_kx10 = np.genfromtxt('fis_225p_d1.84_kx10.txt', delimiter = ' ')
vd_kx10 = data_kx10[:,0] 
...
vd_total = 14 		# cantidad total de velocidades de deseo (vd)
iter_total = 30 	# cantidad total de iteraciones por vd
list_vd = np.linspace(1,7,14)
...
#left, bottom, width, height = [0.17, 0.62, 0.28, 0.28]
...
pylab.ylabel('$t_e$~(s)',size='5.5',labelpad=-5,zorder=1)
pylab.xlim(1.5, 4)
pylab.ylim(19, 25)
ax2.set_yticklabels([])
...
#lgd=plt.legend(numpoints=1,handlelength=0.8) 
ax2.legend(frameon=False,loc='upper center',labelspacing=-0.1,borderpad=0.3,handletextpad=0.5,fontsize=4,numpoints=1) 
...
plt.text(3.9, 18.4, "4", size=4.5)
plt.text(1.2, 24.5, "25", size=4.5)
